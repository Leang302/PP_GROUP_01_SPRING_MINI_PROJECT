...
public class HabitLogController {
    ...
    public ResponseEntity<?> getAllLogsByHabitId(@RequestParam UUID habitId, @RequestParam(defaultValue = "1") @Positive Integer page, @RequestParam(defaultValue = "10") @Positive Integer size) {
        return ResponseEntity.ok().body(ApiResponse.builder().success(true).message("Fetched all habit logs for the specified habit ID successfully!").status(HttpStatus.OK).payload(habitLogService.habitLogService(page, size, habitId)).build());
    }
    ...
    public ResponseEntity<ApiResponse<HabitLog>> createHabitLog(@RequestBody @Valid HabitLogRequest habitLogRequest) {
        ...
        ApiResponse<HabitLog> response = ApiResponse.<HabitLog>builder().success(true).message("Habit log created successfully").status(HttpStatus.CREATED).payload(habitLog).build();
